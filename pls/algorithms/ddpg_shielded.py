...
class DDPG_shielded(DDPG):
    """DDPG wrapper with CleanPLS shield-compatible API."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.exploration_shield is not None and not isinstance(self.action_space, spaces.Discrete):
            warnings.warn(
                "DDPG_shielded received shield_params but action space is not Discrete; "
                "falling back to base DDPG action selection.",
                stacklevel=2,
            )
